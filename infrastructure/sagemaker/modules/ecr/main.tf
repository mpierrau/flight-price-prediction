...
locals {
  watchfiles = ["Dockerfile", "pyproject.toml", "poetry.lock", "nginx.conf"]
  monitor_dir = "${path.root}/${var.src_dir}"
}

resource "docker_image" "this" {
  name = "${aws_ecr_repository.model_repo.repository_url}:${var.ecr_image_tag}"

  build {
    context = "${path.root}/${var.src_dir}"
  }

  triggers = {
    pydir_md5    = md5(join("", [for f in fileset(local.monitor_dir, "**/*.py") : filesha1("${local.monitor_dir}/${f}")]))
    serve_md5    = md5(filesha1("${path.root}/${var.src_dir}/src/serve"))
    watchfiles_md5 = md5(join("", [for f in local.watchfiles : filesha1("${local.monitor_dir}/${f}")]))
  }
}

# Wait for the image to be uploaded before sagemaker config runs
...
resource "docker_registry_image" "this" {
  depends_on = [ aws_ecr_repository.model_repo ]
