"""
This file contain code for engineering new features base on the dataset Kaggle Flight Price Prediction Dataset.
...
def create_weekday_feature(df: pd.DataFrame) -> pd.Series:
    ...
    Args:
        df (pd.DataFrame): Dataframe holding columns "Date", "Month" and "Year" as integers or strings.
...
def create_rounded_arrival_and_departure_times(df: pd.DataFrame) -> tuple[pd.Series,pd.Series]:
    """
    Function for rounding departure hours and arrival hours to closest full hour to reduce cardinality
    Ex. 5:44 -> 6, 23:13 -> 23

    Args:
        df (pd.DataFrame): Dataframe holding columns "Dep_hours" "Dep_min", "Arrival_hours" and "Arrival_min"

    Returns:
        tuple[pd.Series[int],pd.Series[int]]: Tuple holding each Series containing the created feature
    """
    cols = ["Dep_hours","Dep_min","Arrival_hours","Arrival_min"]
    arr_dep_times = df[cols].astype(str)

    dep_closest_full_hour = pd.to_datetime(arr_dep_times["Dep_hours"] + ":" + arr_dep_times["Dep_min"], format="%H:%M").dt.round('h').dt.hour
    arr_closest_full_hour = pd.to_datetime(arr_dep_times["Arrival_hours"] + ":" + arr_dep_times["Arrival_min"], format="%H:%M").dt.round('h').dt.hour
